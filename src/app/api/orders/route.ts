...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    let subtotal = 0;
    let totalDuration = 0;
    const orderItems = validatedData.products.map(item => {
      const product = productMap.get(item.product_id);
      if (!product) throw new Error(`Product not found: ${item.product_id}`);
      
      const itemTotal = product.base_price * item.quantity;
      subtotal += itemTotal;
      totalDuration += (product.estimated_duration_minutes || 60) * item.quantity;
    ...
    const { data: orderData, error: orderError } = await supabase
      ...
      .insert({
        ...
        subtotal,
        total: subtotal, // Add tax calculation logic
